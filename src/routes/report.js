...
// 设备上报：自动注册，无需手动绑定
async function autoBindDevice(deviceId) {
  const device = await prepare('SELECT user_id FROM devices WHERE device_id = ?').get(deviceId);
  if (device) return device.user_id;

  const firstUser = await prepare('SELECT id FROM users ORDER BY id ASC LIMIT 1').get();
...
const checkDevice = asyncHandler(async (req, res, next) => {
  ...
  const userId = await autoBindDevice(deviceId);
  if (!userId) {
...
// 上报应用使用记录（批量）
router.post('/app-usage', checkDevice, asyncHandler(async (req, res) => {
  const { deviceId, records } = req.body;
  if (!records || !Array.isArray(records) || records.length === 0) {
    return res.status(400).json({ error: 'records 不能为空' });
  }

  const insert = transaction(async (items) => {
    let inserted = 0;
  ...
  const insertedCount = await insert(records);

  await prepare('INSERT INTO sync_log (device_id, sync_type, record_count) VALUES (?, ?, ?)')
    .run(deviceId, 'app_usage', insertedCount);

  await prepare('UPDATE devices SET last_online = CURRENT_TIMESTAMP WHERE device_id = ?').run(deviceId);

  res.json({ message: 'ok', count: insertedCount, skipped: records.length - insertedCount });
}));

// 上报网页浏览记录（批量）
router.post('/web-history', checkDevice, asyncHandler(async (req, res) => {
  const { deviceId, records } = req.body;
  if (!records || !Array.isArray(records) || records.length === 0) {
    return res.status(400).json({ error: 'records 不能为空' });
  }
  ...
  const insert = transaction(async (items) => {
    ...
    for (const r of items) {
      ...
      if (!url) continue;
      await prepare('INSERT INTO web_history (device_id, url, title, browser, visited_at) VALUES (?, ?, ?, ?, ?)')
...
// 上报微信小程序使用记录（批量）
router.post('/miniprogram-usage', checkDevice, asyncHandler(async (req, res) => {
  const { deviceId, records } = req.body;
  if (!records || !Array.isArray(records) || records.length === 0) {
    return res.status(400).json({ error: 'records 不能为空' });
  }
...
// 心跳/设备在线
router.post('/heartbeat', checkDevice, asyncHandler(async (req, res) => {
  const { deviceId } = req.body;
  await prepare('UPDATE devices SET last_online = CURRENT_TIMESTAMP WHERE device_id = ?').run(deviceId);
